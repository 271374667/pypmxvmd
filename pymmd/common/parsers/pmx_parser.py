...
class PmxParser:
    ...
    def _setup_parsing_parameters(self, data: bytearray) -> None:
        ...
        # 跳过魔数、版本号和标志数量
        self._io_handler.unpack_data("4s f B", data)
        
        # 读取全局标志
        global_flags = self._io_handler.unpack_data("8B", data)
        ...
        # 恢复数据读取位置
        while self._io_handler.get_position() < current_pos:
            data.insert(0, 0)  # 这是一个简化的位置恢复，实际应用中需要更精确的处理
    ...
    def _parse_materials(self, data: bytearray) -> List[PmxMaterial]:
        ...
        for i in range(material_count):
            ...
            sphere_path = self._io_handler.read_variable_string(data)
            ...
            if toon_flag == 0:
                ...
                toon_path = f"custom_toon_{toon_index}"  # 简化处理
