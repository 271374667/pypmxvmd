...
class PmxModel(PmxRecord):
    ...
    def _validate_data(self, parent_list: Optional[List] = None) -> None:
        ...
        for material in self.materials:
            self._require(
                isinstance(material, PmxMaterial),
                "model.materials",
                "PmxMaterial items",
                material,
            )
        ...
        for i, face in enumerate(self.faces):
            ...
            for vertex_idx in face:
                self._require(
                    isinstance(vertex_idx, int) and 0 <= vertex_idx < vertex_count,
                    f"faces[{i}].vertex_index",
                    f"0..{vertex_count - 1}",
                    vertex_idx,
                )

        if self.materials:
            material_indices = sum(material.face_count for material in self.materials)
            self._require(
                material_indices == len(self.faces) * 3,
                "materials.face_count",
                f"sum equal to {len(self.faces) * 3}",
                material_indices,
            )

        bone_count = len(self.bones)
        for index, bone in enumerate(self.bones):
            self._require(isinstance(bone, PmxBone), f"bones[{index}]", "PmxBone", bone)
            bone.validate(self.bones)
            self._require(
                -1 <= bone.parent_index < bone_count,
                f"bones[{index}].parent_index",
                f"-1..{bone_count - 1}",
                bone.parent_index,
            )
            if bone.tail_bone_index is not None:
                self._require(
                    -1 <= bone.tail_bone_index < bone_count,
                    f"bones[{index}].tail",
                    f"-1..{bone_count - 1}",
            ...
            if bone.bone_flags.inherit_rot or bone.bone_flags.inherit_trans:
                ...
                self._require(
                    isinstance(inherit_parent_index, int)
                    and -1 <= inherit_parent_index < bone_count,
                    f"bones[{index}].inherit_parent_index",
                    f"-1..{bone_count - 1}",
            ...
            if bone.bone_flags.ik:
                ...
                self._require(
                    isinstance(ik_target_index, int)
                    and -1 <= ik_target_index < bone_count,
                    f"bones[{index}].ik_target_index",
                    f"-1..{bone_count - 1}",
                    ik_target_index,
                )
                for link_index, link in enumerate(bone.ik_links):
                    self._require(
                        -1 <= link.bone_index < bone_count,
                        f"bones[{index}].ik_links[{link_index}].bone_index",
                        f"-1..{bone_count - 1}",
                        link.bone_index,
                    )
        ...
        for index, rigid_body in enumerate(self.rigidbodies):
            self._require(
                isinstance(rigid_body, PmxRigidBody),
                f"rigid_bodies[{index}]",
                "PmxRigidBody",
                rigid_body,
            )
            rigid_body.validate(self.rigidbodies)
            self._require(
                -1 <= rigid_body.bone_index < bone_count,
        ...
        for index, joint in enumerate(self.joints):
            self._require(
                isinstance(joint, PmxJoint), f"joints[{index}]", "PmxJoint", joint
            )
            joint.validate(self.joints)
            for field in ("rigidbody1_index", "rigidbody2_index"):
                value = getattr(joint, field)
                self._require(
                    -1 <= value < rigid_body_count,
                    f"joints[{index}].{field}",
                    f"-1..{rigid_body_count - 1}",
                    value,
                )

    def _validate_parent_cycles(self) -> None:
        for start in range(len(self.bones)):
            visited: set[int] = set()
            current = start
            while current != -1:
                if current in visited:
                    raise PmxValidationError(
                        f"bones[{start}].parent_index",
